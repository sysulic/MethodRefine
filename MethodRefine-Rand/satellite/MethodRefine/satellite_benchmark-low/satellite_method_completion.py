...
def get_img__48(state, direction, mode):
	instrument = state.mode[mode][0]
...
def activate__42(state, instrument, satellite, direction):

	return [('switch_on',instrument,satellite),('turn_to',satellite,"direc-2"),('turn_to',satellite,"direc-4"),('calibrate',instrument,satellite,"direc-4"),('turn_to',satellite,direction)], [[1, 2], [2, 3], [3, 4]]
def get_img__49(state, direction, mode):
	instrument = state.mode[mode][0]
...
def activate__43(state, instrument, satellite, direction):

	return [('switch_on',instrument,satellite),('switch_off',satellite),('turn_to',satellite,direction),('calibrate',instrument,satellite,direction)], [[1, 2], [2, 3]]
def get_img__50(state, direction, mode):
	instrument = state.mode[mode][0]

	return [('activate',instrument,state.on_board[instrument],direction),('take_img',state.on_board[instrument],direction,instrument,mode),('switch_off',state.on_board[instrument]),('turn_to',state.on_board[instrument],"direc-3")], [[0, 1], [2, 3]]
def activate__44(state, instrument, satellite, direction):

	return [('switch_on',instrument,satellite),('calibrate',instrument,satellite,"direc-3"),('turn_to',satellite,direction)], [[1, 2]]
def activate__45(state, instrument, satellite, direction):

	return [('switch_on',instrument,satellite),('turn_to',satellite,"direc-3"),('turn_to',satellite,"direc-5"),('switch_off',satellite),('turn_to',satellite,"direc-3"),('turn_to',satellite,direction)], [[1, 2], [2, 3], [3, 4], [4, 5]]
def get_img__51(state, direction, mode):
	instrument = state.mode[mode][0]
