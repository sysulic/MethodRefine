...
import eventlet
import copy
import generateBenchmark_satellite
...
def reduce_methods(all_methods, i):
    ...
    new_methods_list = new_tihtn_planner.generate_methods("satellite", new_all_methods)
	...
	time.sleep(1)
        ...
        for new_method_name in new_methods_list_copy:
            fun_obj = getattr(sys.modules["satellite_method_completion"], new_method_name)
...
def testing(reduced_methods,i):
    ...
    ft.close()
    if ("satellite_method_completion" in sys.modules.keys()):
